...
BUILT=$1
URL="https://circleci.com/api/v1.1/project/github/plotly/plotly.js/$BUILT/artifacts?circle-token=$CIRCLE_TOKEN"
...
for f in $FILES; do
    if [[ $f == *"/test_images/"* ]]; then
        wget $f %$CIRCLE_TOKEN -q --show-progress -P $DIR/test_images
    elif [[ $f == *"/test_images_diff/"* ]]; then
        wget $f %$CIRCLE_TOKEN -q --show-progress -P $DIR/test_images_diff
    fi
done
